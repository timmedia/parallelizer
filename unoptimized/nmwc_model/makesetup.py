...
def maketopo(topo, nxb):
    ...
    topomx2, topomx3, topomx4 = 500, 1000, 1500
    x0_2,x0_3, x0_4 = -100000,0,100000 
    print(x0_4)
    # Calculate Gaussian functions
    toponf1 = topomx * np.exp(-(x / float(topowd)) ** 2)
    toponf2 = topomx2 * np.exp(-((x - x0_2) / float(topowd)) ** 2)
    toponf3 = topomx3 * np.exp(-((x - x0_3) / float(topowd)) ** 2)
    toponf4 = topomx4 * np.exp(-((x - x0_4) / float(topowd)) ** 2)
